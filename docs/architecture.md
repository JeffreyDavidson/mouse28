...
Generate variants after local public-content sync and after publishing/replacing covers. Production generation is an explicitly approved operation requiring `--force`; until then, the new markup safely serves originals. Derived files are additive and are not automatically pruned. Episode-upload variants, explicit edge asset-cache headers, scheduled staging benchmarks and privacy-reviewed field metrics remain follow-up opportunities; do not introduce whole-page shared caching without separating CSRF/session/form-feedback state and publication invalidation.
