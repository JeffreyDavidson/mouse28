        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            @if($latestEpisodes->count())
                    ...
                    @foreach($latestEpisodes as $episode)
                        <a href="/episodes/{{ $episode->slug }}" class="group bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all hover:-translate-y-1 border border-navy/5">
                            <div class="flex items-center gap-3 mb-4">
                                <span class="bg-disney-blue/10 text-disney-blue text-xs font-bold px-3 py-1 rounded-full">EP {{ $episode->episode_number }}</span>
                                @if($episode->duration_seconds)
                                    <span class="text-navy/40 text-xs">{{ $episode->formatted_duration }}</span>
                                @endif
                            </div>
                            <h3 class="font-heading text-xl font-semibold text-navy group-hover:text-disney-blue transition-colors mb-3">{{ $episode->title }}</h3>
                            <p class="text-navy/60 text-sm leading-relaxed line-clamp-3">{{ Str::limit($episode->description, 140) }}</p>
                            <div class="mt-4 text-disney-blue text-sm font-medium flex items-center gap-1">
                                Listen now
                ...
                <div class="text-center mt-10">
                    <a href="/episodes" class="text-disney-blue hover:text-disney-blue-dark font-semibold text-sm transition-colors">View all episodes →</a>
                </div>
            @else
    ...
    <section class="py-16 md:py-24 bg-white">
        <div class="max-w-6xl mx-auto px-4 sm:px-6">
            <div class="grid md:grid-cols-2 gap-12 items-center">
                {{-- Photo placeholder --}}
                <div class="bg-gradient-to-br from-disney-blue/10 to-gold/10 rounded-2xl aspect-[4/3] flex items-center justify-center">
                    <div class="text-center">
                        <span class="text-6xl block mb-3">👨‍👩‍👧</span>
                        <span class="text-navy/30 text-sm">Photo coming soon</span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <a href="/about" class="inline-block mt-6 text-disney-blue hover:text-disney-blue-dark font-semibold text-sm transition-colors">Read our full story →</a>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-16 md:py-24 bg-cream">
        <div class="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            @if($latestPosts->count())
                    ...
                    @foreach($latestPosts as $post)
                        <a href="/blog/{{ $post->slug }}" class="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all hover:-translate-y-1 border border-navy/5">
                            ...
                            <div class="p-6">
                                ...
                                @endif
                                <h3 class="font-heading text-lg font-semibold text-navy group-hover:text-disney-blue transition-colors mt-1 mb-2">{{ $post->title }}</h3>
                ...
                <div class="text-center mt-10">
                    <a href="/blog" class="text-disney-blue hover:text-disney-blue-dark font-semibold text-sm transition-colors">Read more posts →</a>
                </div>
            @else
                <div class="text-center py-12 bg-white rounded-2xl border border-navy/5">
