...
@section('content')
    <section class="bg-gradient-to-br from-navy to-navy-light py-16 md:py-20 relative overflow-hidden">
        {{-- Waveform background --}}
        <div class="absolute inset-0 opacity-[0.05] pointer-events-none flex items-end justify-center gap-[3px] px-4">
            @for($i = 0; $i < 80; $i++)
                <div class="w-1 bg-white rounded-full" style="height: {{ rand(10, 90) }}%"></div>
            @endfor
        </div>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6">
            ...
            @if($episode->audio_url)
                <div class="bg-gradient-to-r from-navy to-navy-light rounded-2xl p-6 md:p-8 shadow-xl mb-10 relative overflow-hidden">
                    ...
                    <div class="absolute inset-0 opacity-[0.06] flex items-center justify-center gap-[2px] pointer-events-none">
                        @for($i = 0; $i < 100; $i++)
                            <div class="w-[2px] bg-white rounded-full" style="height: {{ rand(15, 85) }}%"></div>
                        @endfor
                    </div>
            ...
            <div class="flex flex-wrap gap-3 mb-10">
                @if($episode->apple_url)
                    <a href="{{ $episode->apple_url }}" target="_blank" rel="noopener" class="flex items-center gap-2.5 bg-black text-white font-medium px-6 py-3 rounded-xl shadow-md hover:shadow-lg transition-all text-sm hover:-translate-y-0.5">
                ...
                @if($episode->spotify_url)
                    <a href="{{ $episode->spotify_url }}" target="_blank" rel="noopener" class="flex items-center gap-2.5 bg-[#1DB954] text-white font-medium px-6 py-3 rounded-xl shadow-md hover:shadow-lg transition-all text-sm hover:-translate-y-0.5">
                ...
                @if($episode->youtube_url)
                    <a href="{{ $episode->youtube_url }}" target="_blank" rel="noopener" class="flex items-center gap-2.5 bg-[#FF0000] text-white font-medium px-6 py-3 rounded-xl shadow-md hover:shadow-lg transition-all text-sm hover:-translate-y-0.5">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-navy/5 mb-8">
                <h3 class="font-heading text-lg font-bold text-navy mb-4">Share This Episode</h3>
                <div class="flex flex-wrap gap-3">
                    <a href="https://twitter.com/intent/tweet?text={{ urlencode($episode->title . ' — Mouse28 Podcast') }}&url={{ urlencode(url('/episodes/' . $episode->slug)) }}" target="_blank" rel="noopener"
                       class="inline-flex items-center gap-2 bg-navy/5 hover:bg-navy text-navy/60 hover:text-white px-4 py-2.5 rounded-xl text-sm font-medium transition-all">
                    ...
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url('/episodes/' . $episode->slug)) }}" target="_blank" rel="noopener"
                       class="inline-flex items-center gap-2 bg-navy/5 hover:bg-[#1877F2] text-navy/60 hover:text-white px-4 py-2.5 rounded-xl text-sm font-medium transition-all">
                    ...
                    <button onclick="navigator.clipboard.writeText(window.location.href);this.textContent='Copied!';setTimeout(()=>this.textContent='Copy Link',2000)"
                            class="inline-flex items-center gap-2 bg-navy/5 hover:bg-purple text-navy/60 hover:text-white px-4 py-2.5 rounded-xl text-sm font-medium transition-all">
                ...
                <div class="mt-12">
                    ...
                    <div class="grid sm:grid-cols-2 gap-6">
                        @foreach($relatedPosts as $post)
                            <a href="/blog/{{ $post->slug }}" class="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all border border-navy/5 hover:border-purple/20">
                                @if($post->cover_image)
