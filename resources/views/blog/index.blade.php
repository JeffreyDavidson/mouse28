...
@section('content')
    ...
    <section class="bg-cream border-b border-navy/5" x-data="{ search: '{{ request('q', '') }}' }">
        <div class="max-w-6xl mx-auto px-4 sm:px-6">
            {{-- Search --}}
            <div class="pt-5">
                <form action="/blog" method="GET" class="relative max-w-md mx-auto">
                    @if($category)<input type="hidden" name="category" value="{{ $category }}">@endif
                    <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-navy/30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Search posts..."
                        class="w-full pl-11 pr-4 py-2.5 rounded-full border border-navy/10 bg-white text-navy text-sm placeholder:text-navy/30 focus:outline-none focus:ring-2 focus:ring-purple/30 focus:border-purple transition-all">
                </form>
            </div>
