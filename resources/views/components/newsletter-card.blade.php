<div id="newsletter" class="relative overflow-hidden rounded-2xl border border-white/5 bg-linear-to-br from-navy via-navy-light to-navy p-7 text-center">
    ...
    <div class="relative">
        ...
        <p class="mb-5 text-base/relaxed text-white/40 sm:text-sm/relaxed">{{ $subtitle ?? 'New posts & Disney tips delivered to your inbox' }}</p>

        @if(session('newsletter_success'))
            <p role="status" class="mb-3 rounded-lg border border-green-300/20 bg-green-300/10 px-3 py-2 text-base text-green-100 sm:text-sm">
                You're subscribed! Check your inbox soon.
            </p>
        @elseif(session('newsletter_error'))
            <p role="alert" class="mb-3 rounded-lg border border-red-300/20 bg-red-300/10 px-3 py-2 text-base text-red-100 sm:text-sm">
