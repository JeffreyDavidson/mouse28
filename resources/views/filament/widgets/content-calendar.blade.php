<x-filament-widgets::widget>
    ...
    <div class="relative h-full overflow-hidden rounded-[1.25rem] border border-mouse-gold/15 bg-linear-to-br from-mouse-navy/95 to-mouse-navy-light/90 p-6">
        <div class="mb-5 flex items-center gap-2">
            ...
            <h3 class="m-0 font-mouse-heading text-lg font-bold text-mouse-cream">Upcoming Content</h3>
        ...
        @if(count($items) === 0)
            <div class="px-4 py-8 text-center">
                ...
                <p class="mb-2 font-mouse-heading text-base text-mouse-cream">Your week is wide open!</p>
                <p class="font-mouse-body text-sm text-mouse-gold-light/60">Schedule some magical content for the next 7 days.</p>
            </div>
        @else
            <div class="relative pl-6">
                <div class="absolute inset-y-1 left-[5px] w-0.5 bg-linear-to-b from-mouse-gold/30 to-mouse-purple/30"></div>
                ...
                @foreach($items as $item)
                    ...
                    @endphp
                    <a href="{{ $item['url'] }}" class="relative block rounded-xl pb-4 no-underline transition-colors last:pb-0 hover:bg-mouse-gold/5">
                        ...
                        <div class="rounded-xl px-3 py-2">
                            <p class="font-mouse-body text-sm/5 font-medium text-mouse-cream">{{ $item['title'] }}</p>
                            <div class="mt-1 flex flex-wrap items-center gap-2">
                                <span class="font-mouse-body text-xs text-mouse-gold-light/70">{{ \Carbon\Carbon::parse($item['date'])->format('M j, g:ia') }}</span>
