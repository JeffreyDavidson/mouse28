<x-filament-panels::page>
    ...
    @if ($count === 0 && ! $error)
        <div class="bg-mouse-navy-light/30 border-mouse-gold/12 rounded-2xl border px-6 py-16 text-center">
            ...
            <p class="font-mouse-body text-mouse-cream/50 mt-2 text-sm">Share your newsletter signup link to start growing your audience ✨</p>
        </div>
    @elseif ($count > 0)
        <div class="bg-mouse-navy-light/30 border-mouse-gold/12 overflow-x-auto rounded-2xl border">
            <table class="w-full min-w-160 border-collapse">
                <thead class="bg-mouse-navy/60">
                    <tr>
                        <th class="border-mouse-gold/10 font-mouse-body text-mouse-gold/80 border-b px-6 py-4 text-left text-xs font-semibold tracking-wider uppercase" scope="col">#</th>
                        <th class="border-mouse-gold/10 font-mouse-body text-mouse-gold/80 border-b px-6 py-4 text-left text-xs font-semibold tracking-wider uppercase" scope="col">Email Address</th>
                        <th class="border-mouse-gold/10 font-mouse-body text-mouse-gold/80 border-b px-6 py-4 text-left text-xs font-semibold tracking-wider uppercase" scope="col">Subscribed</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($subscribers as $index => $subscriber)
                        ...
                        <tr class="hover:bg-mouse-gold/4 transition-colors">
                            <td class="border-mouse-gold/6 font-mouse-body text-mouse-cream/40 border-b px-6 py-4 text-xs">{{ $index + 1 }}</td>
                            <td class="border-mouse-gold/6 border-b px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <span class="font-mouse-body from-mouse-purple text-mouse-gold-light to-mouse-navy-light flex size-9 shrink-0 items-center justify-center rounded-full bg-linear-to-br text-xs font-semibold" aria-hidden="true">
